...
- `DELETE /api/templates/{tid}`（id 正则白名单防路径穿越，删对应 YAML）。`app.js` 加 `API.deleteTemplate`。
- 工作台规范卡 hover 显示**删除按钮**，`confirm` 后删除、刷新列表、必要时改选中项。
- **解依赖**：编译器 schema 样例从"读农大文件"改为内置 `template_norm.DEFAULTS` 骨架——任何模板（含农大）都能安全删，不会弄坏编译；样例也更中性。
- **修缓存**：`no-cache` 中间件从"仅 /static"扩到"所有非 /api 响应"，HTML 页面前端改动即时生效（之前 index.html 被浏览器缓存，新删除按钮不出现）。
- 验证：DELETE 删掉 custom-thesis-42f448（列表+磁盘消失）；工作台 3 卡（USTC/农大/新疆）各带删除按钮。

## 2026-06-18 · 修：删除/编译触发 uvicorn 重启致界面打不开（稳定性）
