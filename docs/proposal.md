...
```
建设期（离线，决定可扩展性）
  规范文档(PDF/Word) → 规范编译器(LLM抽规则) → 声明式模板(YAML) → 算力自检(0人工) → 入库
                                                                         │
                                                          ┌──────────────▼──────────────┐
                                                          │   模板库（核心资产 / 数据层）   │
                                                          │  可声明 · 可维护 · 可扩展 · 自增长 │
...
- **不是 agent，是 workflow**：控制流固定可知，LLM 当组件不当司机。线上一条确定性流水线 + 一个有上限的「校验→不过回炉」循环；建设期一个会用工具的单 agent + 算力自检闸（0 人工：多模型投票 + 回译交叉校验）。**不上多 agent 团队**——它会把成本、延迟、不确定性全推向你最不想要的方向，直接破坏「确定性 + 低成本 + 输出可控合规」。

---

## 3. 多规范适配机制（核心）

> 目标：新增一份规范 = 上传它的格式要求 + **算力自检一次**（多模型投票 + 回译交叉校验），**0 人工、不写代码**。
...
| 模板（"本校一级标题=黑体小二号段前0.5行…"）| **每份规范一份**，纯数据 | 规范编译器 + 算力自检（0人工） |
...
然后**算力自检一遍**（最强模型多次投票 + 把抽出的 YAML 回译成人话，与样张/规范原文逐条交叉比对，挑出抽错/抽漏的字段并自纠或标存疑），过了即入库——**0 人工**。之后用这份规范的所有文档都自动套用。**这道「回译交叉校验」专堵三道闸查不到的「规范本身被读错」——用算力换人工。**
...
```
上传规范 → 规范编译器产 draft(YAML + reference.docx) → 算力自检(投票+回译交叉校验, 0人工) → 入库
         （写入 templates/<规范>/、更新 registry.json、打 version tag）→ 上线
```

- **可选 · 提交入口**：开放「提交我的规范」，用户上传规范 → 编译器产 → 算力自检（0人工） → 入库。模板库**自增长**。
...
| ⑥ 校验 / 合规报告 | **三道闸**：格式断言（驱动回炉）+ 内容守恒（每元素，掉信息就停下）+ 结构校验（拿图/表事实反查 LLM 标签）| 借 **PaperFormatDetect** 思路 |
| ⑦ 视觉 QA（第四张网，all-in 该做） | 成品 LibreOffice→PDF→视觉模型挑视觉毛病（错位/压字/孤行）| **只进报告、永不改稿** |
| 编排 | 纯 Python 状态机即可（线性 + 一个重试环） | 要显式重试图用 **LangGraph**；编译器那步可选 **PydanticAI** |
| 界面 | 自用 **Gradio** / CLI；服务化 **FastAPI**；可包成 **MCP** 从 Claude 驱动 | — |

**LLM 路线 = All-in 求最优效果**：两个缝（结构识别、规范抽取）上最强 API 模型、喂全文与上下文、分类多跑投票取一致；不设隐私/算力门槛。封在 Instructor/Pydantic 接口后保留切本地口子，但 v1 不作为功能。

**模型策略（All-in，取代原成本控制）**：不做「便宜模型分类、低置信度才升级」的省钱降级。钱花在**「更多检查网」**（分类投票 + 视觉 QA 第四张网），**不花在「更多驾驶」**——LLM 永不当流水线司机、永不碰内容。这是「最好效果 = 可复现的合规 + 内容零损」的来源：质量靠确定性给，不靠把活儿交给 LLM 给。
...
- **数据隐私（v1 已降级）。** 处理的是未发表论文 + 个人信息，确属敏感；但 v1 明确 **all-in 走最强 API 求最优效果，不设隐私门槛**。接口层保留切本地口子，待真有隐私敏感需求再 productize。
